    ...
    public class BaseNodeView : NodeView
    {
        ...
        protected virtual void DrawDefaultInspector(bool fromInspector = false)
        {
            ...
            foreach (var field in fields)
            {
                ...
                #if ODIN_INSPECTOR
                // TODO: ShowIfAttribute
                // Show the field only if the condition is met
                if (field.GetCustomAttribute(typeof(ShowIfAttribute)) is ShowIfAttribute { Condition: { } } showIfAttribute)
                {
                    var conditionField = nodeTarget.GetType().GetField(showIfAttribute.Condition);
                    if (conditionField != null)
                    {
                        var conditionValue = conditionField.GetValue(nodeTarget);
